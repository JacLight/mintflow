...
import aiModels from './ai-model-map.js';


const aiProviders = Array.from(new Set(aiModels.map((model) => model.provider)))
...
const aiPlugin = {
  ...
  icon: 'Brain',
  description:
    'A plugin to interact with various AI models through APIs and Ollama',
    ...
    properties: {
      ...
      model: {
        ...
        dataSource: {
          source: 'json',
          value: aiModels,
          filter: { property: 'provider', operator: 'equals', value: '{{provider}}' },
        }
      },
        ...
        if (input.provider) {
          ...
          for (const providerType of providers) {
            try {
              ...
              console.error(
                `Error fetching models from ${providerType}:`,
                error
              );
          ...
          switch (provider) {
            case 'openai':
              ...
              if (config.providers.openai.organization) {
                authHeaders['OpenAI-Organization'] =
            ...
            case 'anthropic':
              ...
              authHeaders = {
                ...
                'Content-Type': 'application/json',
                'anthropic-version':
                  config.providers.anthropic.apiVersion || '2023-06-01',
              };
              break;
            ...
            case 'google':
              ...
              if (config.providers.google.apiKey.startsWith('AIza')) {
                ...
                authHeaders[
                  'Authorization'
                ] = `Bearer ${config.providers.google.apiKey}`;
              }
              break;
