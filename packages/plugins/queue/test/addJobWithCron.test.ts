...
describe('Queue Plugin - addJob with cron options', () => {
  let mockQueue: any;
  let mockJob: any;
  ...
  it('should add a job with cron repeat options', async () => {
    ...
    expect(mockQueue.add).toHaveBeenCalledWith('cronJob', { test: 'data' }, {
      repeat: {
        cron: '0 0 * * *',
        tz: 'America/New_York'
      }
