...
export const BaseNode = memo(
  ({
    ...
    const calculatedSourcePosition = useMemo(() => {
      if (
        typeof data.sourcePosition === 'object' &&
    ...
    const calculatedTargetPosition = useMemo(() => {
      if (
        typeof data.targetPosition === 'object' &&
    ...
    return (
      <div
        className={classNames(
          `rounded-md border border-gray-200 bg-background shadow-md transition-all`,
          selected ? 'ring-2 ring-purple-700' : '',
          className
        )}
      >
